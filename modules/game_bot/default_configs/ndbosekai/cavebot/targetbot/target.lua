UI.Separator()
UI.Label("--- Target Editor ---"):setColor('#EBDAA2')
UI.Separator()
...
targetbotMacro = macro(100, function()
  local ignore = {'[PET] Konpachi','[PET] Kon','[PET] Rinrin','[PET] Noba','[PET] Kurodo'}
  ...
  for i, creature in ipairs(creatures) do
    local path = findPath(player:getPosition(), creature:getPosition(), 7, {ignoreLastCreature=true, ignoreNonPathable=false, ignoreCost=true})
    if creature:isMonster() and (oldTibia or creature:getType() < 3) and path then
      ...
      if params.priority > 0 then
        ...
        if params.priority > highestPriority then
          ...
          if not isInArray(ignore, creature:getName()) then highestPriorityParams = params end
        end
